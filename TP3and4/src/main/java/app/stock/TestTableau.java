...
public class TestTableau {
  public static void main(String[] args) {
    ...
    if (!s.isEmpty())
      System.out.println("Erreur methode isEmpty()");
    if (s.isFull())
      System.out.println("Erreur methode isFull()");
    if (s.getSize() != 0)
      System.out.println("Erreur methode getSize()");
    try {
      s.remove();
    ...
    try {
      s.remove();
      if (s.getSize() != 4)
        System.out.println("Erreur methode getSize() apres remove()");
    } catch (StockEmpty e) {
    ...
    try {
      if (!s.remove(lait))
        System.out.println("Erreur remove(lait) n'a pas trouve le lait a enlever ");
      ;
      if (s.getSize() != 3)
        System.out.println("Erreur remove(lait) n'a pas ote le lait du  stock ");
    ...
    try {
      if (s.remove(lait))
        System.out.println("Erreur remove(lait) a  pu oter le lait qui devait avoir ete ote precedemment ");
      ;
      if (s.getSize() != 3)
        System.out
            .println("Erreur remove(lait) avec du lait qui avait ete ote precedemment ne devait pas modifier size ");
    ...
    try {
      s.remove(fromage);
      if (s.getSize() != 2)
        System.out.println("Erreur remove(fromage) n'a pas ote le fromage du stock ");
    ...
    try {
      ...
      if (s.getSize() != 2)
        System.out.println(
            "Erreur remove(jambon2) n'a pas leve une Exception alors que le produit jambon2 n'est plus dans le stock ");
    } catch (Exception e) {
